	...
	if(text) {
		...
		var user = number + '@s.whatsapp.net'
			...
			if(global.DATABASE._data.users[user].whitelist == true){
			  conn.reply(m.chat, `*@${user.split('@')[0]} sudah ada di daftar putih*`, m, {contextInfo: {
          mentionedJid: [user]
        }})
			}else {
        global.DATABASE._data.users[user].whitelist = true
				conn.reply(m.chat, `*@${user.split('@')[0]} pengguna berhasil ditambah ke daftar putih*`, m, {contextInfo: {
          mentionedJid: [user]
        }})
			}
	}	
...
handler.help = ['<nomor>','<@user>','<reply>'].map(v => 'whitelist ' + v)
handler.tags = ['owner']
